...
public class BlockMutator extends Visitor {
    ...
    private BlockMutator(GNode n, ArrayList<String[]> parameters, String className, boolean isMain) {
    ...
    public static BlockMutator getBlock(GNode n, ArrayList<String[]> parameters, String className, boolean isMain) {
        BlockMutator block = new BlockMutator(n, parameters, className, isMain);
        return block;
    }
    ...
    public void visitExpression(GNode n) {
        if(!isMain) {
            ...
            if(n.getNode(0).getName().equals("SelectionExpression")) {
                if(n.getNode(0).getNode(0).getName().equals("ThisExpression")) {
                    newExpression = newExpression + "__this->";
                    String firstPI = n.getNode(0).getString(1);
                                ...
                                if (n.getNode(3).getName().equals("Arguments")) {
                                    ...
                                    if (Arguments.size() > 0) {
                                        if (Arguments.getNode(0).getName().equals("PrimaryIdentifier")) {
                                            ...
                                            callExpression.add("__this->" + primId);
...
//                    if(returnValue.equals(fields.get(i).fieldMember[1])){
//                        newReturn = "__this->";
//                    }
//                }
                newReturn = newReturn + "__this->" + returnValue + "->__vptr->" + methodName + "(__this->" + returnValue + ")";
            } else {
                returnValue = n.getNode(0).getString(0);
                newReturn = newReturn + "__this->" + returnValue;
            }
