...
public class ClassBodyMutator extends Visitor {
    ...
    public static ClassBodyMutator regularClass(GNode n, String className, String classExtension, ChildToParentMap map, boolean isMain) {
        ...
        if(newClass.initConstructors.isEmpty()) {
            newClass.writeDefaultConstructors();
        }
    ...
    public void handleMethods(GNode n) {
        new Visitor() {
            public void visitMethodDeclaration(GNode n) {
                methods.add(MethodDeclarationMutator.methodSignatureInfo(n, className,isMain,fields)); // add each new method signature to method list
...
//                MethodDeclarationMutator.mutateMethod(n); // call method to mutate the node to reflect C++
    ...
    public void handleConstructors(GNode n) {
        new Visitor() {
            public void visitConstructorDeclaration(GNode n) {
                ...
                initConstructors.add(ConstructorDeclarationMutator.initSignatureInfo(n, className, fields, classExtension, isMain));
    ...
    // if no constructors are specified, write a default one
    public void writeDefaultConstructors() {
        this.initConstructors.add(ConstructorDeclarationMutator.defaultInitSignatureInfo(className, fields, classExtension,isMain));
        this.initConstructors.get(initConstructors.size()-1).addImplicitThis();
    }
